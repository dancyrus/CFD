...
```
L_c15 = ( r_t(√ε − 1) + 1.5·r_t(sec 15° − 1) ) / tan 15°     [1.5 R_t is H&H's REFERENCE arc]
...
⚠ **The reference cone and the nozzle are two different cones.** The bell's own downstream arc stays 0.382 R_t (0.300 for Raptor 2); only the *reference* cone the percentage is measured against takes 1.5 R_t. The app's `Conical` contour is drawn with 0.382 R_t, so an "80% bell" is 80.4% of the cone the app itself draws — deliberate, and the two must not be conflated.

⚠ **The reference cone's throat arc is 1.5 R_t, not the nozzle's own 0.382 R_t.** Huzel–Huang measure bell length against a 15° cone of the same throat area and area ratio **with a 1.5 R_t downstream throat arc**; that arc is part of the *definition of the percentage* and does not track the nozzle being designed. Reading it from the spec instead (which this build did until the contour-hardening pass — code and this line both said 0.382) shortens every bell: RS-25's 80% length error against the published 121 in / 10.88 in moves from −1.74% to −1.21%, and an engine with a tighter arc (Raptor 2, 0.300 R_t) would otherwise get a *different* reference length from the same "80%".

⚠ **Two circulating definitions of L_c15 differ by an ε-dependent 0.4–2.9%.** The Huzel–Huang form above includes the throat-arc term; Aspirespace and the widely-copied `bell_nozzle.py` drop it. With the 1.5 R_t reference arc the gap is 2.89% at ε = 8, 1.32% at ε = 25, 0.72% at ε = 69, 0.45% at ε = 165 — it is *not* the constant 0.337% this section used to quote, which was the ε = 25 value of the (incorrect) 0.382 R_t form. Pick the form above and state it, or "80% bell" is ambiguous.

**Direct-angle bell** (`ContourKind::DirectBell`). Same construction, but θ_n and θ_e are inputs and the downstream arc is the spec's, for measured hardware the Rao table cannot represent. Raptor 2 is the case in hand: published θ_n 32.0°, θ_e 6.0°, downstream arc 0.300 R_t (FAA Analysis Report 2019-001b Table 1). Measured over the digitised range at ε = 34.3 (`rao_table_cannot_represent_raptor2`, which records the sweep): the table reproduces θ_n at a 75.0% bell, but θ_e is 3.64° off there, and the percent that minimises the *worse* of the two angles (82.1%) still misses by 1.85°. No bell percent satisfies both — a 6° exit sits on the 90% row, where θ_n is wrong by ~3°. This is a contour-family mismatch, **not** a γ effect: Rao (1958) states γ barely moves the contour at fixed length and area ratio, and the sign is inverted anyway — γ 1.16 against the chart's 1.23 makes the exit angle *larger*, and by well under 0.7°. `length_fraction` still measures against L_c15 above, because two wall angles and an exit radius do not fix a length.
...
⚠ The floor rule was binary ("anything at all while the counter is nonzero") when the floor sat at 1e-8. At the raised 1e-6·p₀ floor (§5) the cold-start front of the high-pressure presets *legitimately* passes below the floor — measured: Raptor 2 ~600 activations ending by step 130, RS-25 ≤ 2.9k ending by step 220, Merlin Vac ≤ 149k ending by step 796, all engines zero activations ever after, and ambient transitions add exactly zero — so a permanently-poisoned counter would blank three of the six presets forever for a transient whose invented mass leaves the domain in milliseconds. The graded Long plume domain's sea-level cold start is in the same class (measured: 552 activations at the lip corner, steps 91–121 — the startup blast expands deeper before relief in the larger domain — zero ever after; the compact and Standard domains stay clean). The quarantine form keeps the spirit: while activations are recent the field is being invented and nothing displays; after 1500 quiet steps (≈ 2× the longest observed activation window) the report returns **with a permanent amber disclosure** of the count — the event is never hidden, and a floor that fires at steady state keeps the report blank indefinitely.
