...
**Direct-angle bell** (`ContourKind::DirectBell`). Same construction, but θ_n and θ_e are inputs and the downstream arc is the spec's, for measured hardware the Rao table cannot represent. Raptor 2 is the case in hand: published θ_n 32.0°, θ_e 6.0°, downstream arc 0.300 R_t (FAA Analysis Report 2019-001b Table 1). At ε = 34.3 the table's θ_n lands within 0.16° at 75.6% but its θ_e misses by 3.47°, and no bell percent satisfies both — a 6° exit sits on the 90% row. This is a contour-family mismatch, **not** a γ effect: Rao (1958) states γ barely moves the contour at fixed length and area ratio, and the sign is inverted anyway — γ 1.16 against the chart's 1.23 makes the exit angle *larger*, and by well under 0.7°. `length_fraction` still measures against L_c15 above, because two wall angles and an exit radius do not fix a length.
